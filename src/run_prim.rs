...
pub fn run_prim_mod(prim: &Primitive, mut ops: Ops, env: &mut Uiua) -> UiuaResult {
    match prim {
        ...
        Primitive::Tuples => tuples::tuples(ops, None, env)?,
...
impl ImplPrimitive {
    pub(crate) fn run(&self, env: &mut Uiua) -> UiuaResult {
        match self {
            &ImplPrimitive::DeshapeSub(i) => {
                env.monadic_mut_env(|val, env| val.deshape_sub(i, 0, true, env.ctx()))?
            }
            ...
            ImplPrimitive::ATan => env.monadic_env(Value::atan)?,
            ImplPrimitive::ASinH => env.monadic_env(Value::asinh)?,
            ImplPrimitive::ACosH => env.monadic_env(Value::acosh)?,
            ImplPrimitive::ATanH => env.monadic_env(Value::atanh)?,
    ...
    pub(crate) fn run_mod(&self, ops: Ops, env: &mut Uiua) -> UiuaResult {
        match self {
            ...
            &ImplPrimitive::SidedTuples(side) => tuples::tuples(ops, Some(side), env)?,
            ...
            &ImplPrimitive::EachSub(n) => {
                ...
                for mut val in vals {
                    val.deshape_sub(n + 1, 0, val.rank() == max_rank, env.ctx())?;
                    env.push(val);
                }
                zip::rows(f, 0, false, env)?;
    ...
    REGEX_CACHE.with(|cache| -> UiuaResult {
        ...
        for caps in regex.captures_iter(&target) {
            let row: EcoVec<_> = caps
                ...
                .collect();
            matches.append(row.into(), false, env.ctx())?;
            locations.append(
                (caps
                    .get(0)
