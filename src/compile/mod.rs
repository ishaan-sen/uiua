...
impl Compiler {
    ...
    fn try_(&mut self, branches: Vec<Sp<Word>>, span: CodeSpan, pattern: bool) -> UiuaResult<Node> {
        ...
        let ops = (branches.into_iter())
            .map(|w| self.word_sig(w))
            .collect::<UiuaResult<EcoVec<_>>>();
        self.in_try = in_try;
        let mut ops = ops?;
        let (try_sig, _) = try_sig(&ops);
        for op in ops.make_mut() {
            if op.node.is_noreturn(&self.asm) {
                op.sig.update_args_outputs(|a, o| {
                    (
                        a.max(try_sig.args().saturating_sub(1)),
                        o.max(try_sig.outputs()),
                    )
                });
            }
        }
